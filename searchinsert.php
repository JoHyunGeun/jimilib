...
$result = $conn->query("SELECT* FROM reserv WHERE title like '$a%'");
...
$reservid2='1';
											...
											if ($reservid2) {
											$resu = $conn->query("SELECT* FROM film WHERE title like '$a%'");
	...
	$resul = $conn->query("INSERT INTO rental (film_id, name,rental,back) values('$a','$b','$c','$d')");
	$rl = $conn->query("INSERT INTO rentalrecord (film_id, name) values('$a','$b')");

	}
	else{
		echo("
					<script>
						window.alert('없는 도서입니다.')
						history.go(-1)
					</script>
				");
	}
